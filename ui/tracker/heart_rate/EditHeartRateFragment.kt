...
@AndroidEntryPoint
class EditHeartRateFragment : BaseFragment<FragmentAddHeartRateBinding>() {
    ...
    override fun initializeEvent() {
        binding.btnBack.setOnClickListener {
            if (AddHeartRateFragment.isFromTracker) {
        ...
        binding.btnSave.setOnClickListener {
            if (isEdit) {
                ...
                Toast.makeText(
                    requireContext(),
                    getString(R.string.edit_heart_rate_success), Toast.LENGTH_SHORT
                ).show()
            } else {
                viewModel.addHeartRate()
                Toast.makeText(
                    requireContext(),
                    getString(R.string.add_heart_rate_success), Toast.LENGTH_SHORT
                ).show()
            }
        }
        ...
        viewModel.isSaveSuccess.observe(viewLifecycleOwner) {
            ...
            intent.putExtra(Utils.KEY_TRACKER, 2)
            intent.putExtra(
                Utils.KEY_DATA,
...
//        viewModel.initData(requireContext())
    ...
    private fun bindViewHeartRate() {
        ...
        binding.npkBPM.value = heartRate.heartRate
